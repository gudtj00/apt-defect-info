...
def part_divider(number, title, subtitle, label_text=None):
    label = P(label_text if label_text else f"PART {number}", "part_label")
...
def objective_box(items):
    """학습목표 박스."""
    body = [P("이 강의를 마치면 다음을 할 수 있습니다.", "card_sub")]
    for it in items:
        body.append(P(f"☐ {it}", "card_body"))
    return card("학습목표", body, bg=BLUE_LIGHT_BG, pad=12)
...
def summary_box(items):
    ...
    for it in items:
        body.append(P(f"• {it}", "card_body"))
    return card("핵심 정리", body, bg=GREEN_BG, pad=12)


def practice_box(text):
    """설명 연습(다른 사람에게 설명하기) 박스."""
    return card("설명 연습 — 이렇게 말해보세요", [P(text, "card_body")], bg=colors.HexColor("#f0eaf9"), pad=12)


def defect_card(article_no, title, criterion, plain, years, extra=None):
    """38개 하자유형 압축 카드: 조문+제목 / 판정기준 원문 / 쉬운설명 / 담보기간 배지."""
    header = Table([[P(f"<b>{article_no} {title}</b>", "card_title"),
                      pill(f"{years}", BLUE_LIGHT_BG, NAVY)]],
                    colWidths=[CONTENT_WIDTH - 2 * 10 - 44, 44])
    header.setStyle(TableStyle([
        ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
        ("LEFTPADDING", (0, 0), (-1, -1), 0), ("TOPPADDING", (0, 0), (-1, -1), 0),
        ("BOTTOMPADDING", (0, 0), (-1, -1), 0), ("ALIGN", (1, 0), (1, 0), "RIGHT"),
    ]))
    body = [header, Spacer(1, 4), P(f"<i>{criterion}</i>", "card_sub"), P(plain, "card_body")]
    if extra:
        body.append(P(extra, "caption"))
    return card("", body, bg=GRAY_LIGHT, pad=10, title_style="card_title")


def chapter_header(num, title, subtitle):
    return part_divider(num, title, subtitle, label_text=f"제{num}강")
