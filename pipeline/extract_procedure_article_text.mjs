#!/usr/bin/env node
// Phase 4 축② 상세 페이지용. raw/law/ 원문 텍스트에서 제45조~제124조(조사방법·보수비용 조문) 전문을
// grep/파싱으로 그대로 추출한다 — 기억이나 요약으로 재작성하지 않고 원문 그대로 옮긴다.
// 산출물: data/procedure-article-text.json ({ "84": "제84조(...) ...", ... })
...
const OUT_FILE = path.join(ROOT, "data", "procedure-article-text.json");
...
for (const line of lines) {
  ...
  if (currentCode !== null) {
    if (CHAPTER_HEADER.test(line)) continue; // 장 제목은 조문 본문이 아니므로 제외
    buffer.push(line);
  }
}
flush();
  ...
  JSON.stringify(
    {
      ...
      note: "raw/law/ 원문 텍스트에서 제45조~제124조(조사방법·보수비용 조문) 전문을 그대로 추출함(요약·재작성 없음).",
      articles,
    },
    null,
    2,
