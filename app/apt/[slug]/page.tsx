...
const STATUS_STYLE: Record<WarrantyLineItem["status"], string> = {
  ...
  expiring_soon: "bg-amber-50 text-amber-700 ring-amber-100",
  expired: "bg-slate-100 text-slate-500 ring-slate-200",
};

function ItemRow({ item }: { item: WarrantyLineItem }) {
  return (
    <tr className={`border-t border-slate-100 ${item.status === "expired" ? "opacity-60" : ""}`}>
      <td className="px-4 py-3 font-medium text-slate-900">{item.categoryName}</td>
      <td className="px-4 py-3 text-center text-slate-500">{item.years}년</td>
      <td className="px-4 py-3 text-slate-500">{item.expiryDate}</td>
      <td className="px-4 py-3">
        <span
          className={`inline-block rounded-full px-2.5 py-1 text-xs font-semibold ring-1 ${STATUS_STYLE[item.status]}`}
        >
    ...
    <div>
      ...
      <section className="border-b border-slate-100 bg-white">
        <div className="mx-auto max-w-3xl px-5 py-10 text-center sm:py-14">
          ...
          <h1 className="text-2xl font-extrabold tracking-tight text-slate-900 sm:text-4xl">{complex.name}</h1>
          <p className="mt-3 text-slate-500">{complex.address ?? complex.region}</p>
      ...
      <div className="mx-auto max-w-3xl px-5 py-10">
        {!result.ok ? (
          ...
          <>
            <section>
              <h2 className="text-lg font-bold text-slate-900">공용부분</h2>
            ...
            <section className="mt-10 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              <h2 className="text-base font-bold text-slate-900">전유부분 실제 인도일을 아시나요?</h2>
              ...
              <details className="mt-3 text-sm">
                ...
                <div className="mt-3 space-y-3 text-sm leading-7 text-slate-600">
                  <p>
                    <strong className="font-semibold text-slate-900">이 날짜가 뜻하는 것</strong> —
                  ...
                  <p className="font-semibold text-slate-900">확인 방법 (확실한 순서대로)</p>
                  <ol className="list-decimal space-y-2 pl-5">
                    <li>
                      <strong className="font-semibold text-slate-900">등기부등본의 &#39;소유권보존등기일&#39;</strong> —
                      인터넷등기소에서 누구나 발급 가능합니다. 실제 인도일과 정확히 같지는 않을 수 있지만 통상 비슷한
                      시기라 가장 접근하기 쉬운 근사치입니다.
                    </li>
                    <li>
                      <strong className="font-semibold text-slate-900">관리사무소 문의</strong> — 관리주체는
                      사업주체로부터 세대별 인도일을 넘겨받아 등록할 법적 의무가 있습니다(공동주택관리정보시스템 운영
                      관리규정 제15조④). 다만 실제 열람 절차나 제공 여부는 단지마다 다를 수 있습니다.
                    </li>
                    <li>
                      <strong className="font-semibold text-slate-900">주택인도증서 원본</strong> — 최초 입주자가 입주
              ...
              <form method="get" className="mt-4 flex flex-wrap items-center gap-2">
                ...
                {handoverDate && (
                  <Link href={`/apt/${slug}`} className="text-sm text-slate-500 no-underline hover:text-slate-700">
                    초기화
                  </Link>
                )}
              </form>
              ...
              {result.jeonyuHandoverDateError && (
                <p className="mt-2 text-sm text-red-600">{result.jeonyuHandoverDateError}</p>
              )}
        ...
        <div
          className={`mt-10 rounded-2xl border px-5 py-5 ${
            anyClaimable ? "border-emerald-200 bg-emerald-50" : "border-slate-200 bg-white"
          }`}
        >
          <strong className={`block text-base font-bold ${anyClaimable ? "text-emerald-900" : "text-slate-900"}`}>
          ...
          <Link
            href="/claim"
            className="mt-4 inline-block rounded-full bg-emerald-600 px-5 py-2.5 text-sm font-semibold text-white no-underline hover:bg-emerald-700"
          >
            하자보수 청구 절차 보기
          </Link>
        </div>

        <p className="mt-6 text-xs leading-6 text-slate-400">{CALC_DISCLAIMER}</p>
      </div>
    </div>
  );
}
